...
Hologram_resolution_y = 512;  % Hologram resolution
    ...
    for color_index = 1:length(lambda)
        ...
        for layer = 1: length(Cut)
            ...
            elseif N > active_area_limit
                ...
                t = 1;
                wrp_no = wrp_no + 1 ;
                z_wrp = abs(layer_diff/2+Cut(layer));
        ...
        for nwrp = 1:length(depth_ranges)
            ...
            for layer = 1: length(depth_range)
                ...
                %
                N = round(abs(z).*tan(lambda(color_index)./(2.*Hologram_sampling_interval))./Hologram_sampling_interval);
...
%                       imwrite(uint8(rot90((original(:,:,color_index)))*10),fullFileName);
    ...
    file_name = strcat('papili','_d_',num2str(d),'WRP_',num2str(length(depth_ranges)),'_total_layer_',num2str(length(Cut)),'_recon_d_',num2str(d2),num2str(active_area_limit),file_type);
    fullFileName = fullfile(sub_dir,file_name);
