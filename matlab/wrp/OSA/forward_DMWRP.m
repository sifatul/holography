...
while segmentation_number < 84%125
...
for color_index = 1:length(lambda)  %iterate each color
    ...
    for depth_range_index = 1:length(C)       %iterate each depth range
        ...
        WRP_index = B(depth_range_index);
        
        wrp_location = uv(WRP_index)-0.000005;  %  WRP location    
        d_wrp_to_hologram = max(abs(d-wrp_location));
%         E(depth_range_index)   
        for depth_index = B(depth_range_index):E(depth_range_index)     %iterate each depth layer
            
%             object_point_index = object_point_index +1 
            indexes = find(obj(:,3) == uv(depth_index)); %indexes with this number of object point
           
            pObj = obj(indexes,:);
            z =  (abs(wrp_location -  uv(depth_index)));
