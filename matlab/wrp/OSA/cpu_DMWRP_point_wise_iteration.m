...
%% 
% three_object highest 55 segmentation can be done
% % obj(:,1) = obj(:,1)*3.5; 
% obj(:,1) = obj(:,1)*3.5; 
% obj(:,2) = obj(:,2)*3.5;  
% obj(:,3) = obj(:,3)*50;   
...
%         E(depth_range_index)   
        depth_index = B(depth_range_index):E(depth_range_index);     %iterate each depth layer        
        A= obj_z( ismember( obj_z, uv(depth_index) ) ); 
        [Lia, Locb]   = ismember(obj_z,uv(depth_index), 'rows');
        indexes= find(Locb ~= 0);
        z =  ((wrp_location -  uv(depth_index)));
        ...
        for Ni = 1 : length( indexes)   %iterate each object point
            ...
            [y_run, x_run]= meshgrid((-(active_area(Ni)-1)/2:(active_area(Ni)-1)/2)*Hologram_sampling_interval,(-(active_area(Ni)-1)/2:(active_area(Ni)-1)/2)*Hologram_sampling_interval);
            r =  sign(z(Ni))*sqrt(z(Ni).^2 + y_run.^2 + x_run.^2); 
            ...
            WRP(nx:nx+active_area(Ni)-1,ny:ny+active_area(Ni)- 1)= WRP(nx:nx+active_area(Ni)-1,ny:ny+active_area(Ni)-1) + Sub_hologram;
